...
class KWSDetection(object):
    ...
    def __init__(self):
        ...
        self.continuous_pub_ = rospy.Publisher("jsgf_audio", String, queue_size=10)

        # initialize node
        rospy.init_node("sentrybot_kws_control")
        # Call custom function on node shutdown
        rospy.on_shutdown(self.shutdown)

        # Params
        _root = rospkg.RosPack().get_path("sentrybot_audio")
        print(_root)
        ...
        if rospy.has_param(_hmm_param):
            ...
            if rospy.get_param(_hmm_param) == ":default":
              ...
              self.class_hmm = _root + "/" + self.class_hmm
        else:
        ...
        if rospy.has_param(_dict_param) and rospy.get_param(_dict_param) != ":default":
            self.lexicon = _root +"/" + rospy.get_param(_dict_param)
        else:
            rospy.logerr(
